...
1. Aby program działał musisz mieć pliki:
	i) konfiguracyjny: config.yaml
	ii) python: wchodzenie_pobieranie.py, wczytywanie_analizowanie.py, functions.py, sending_e-mail.py (do wysyłki maila, nie jest obowiązkowy, jeśli chcesz go używać to na razie w kodzie należy zmienić odbiorcę)
	iii) excel: data.csv (z odpowiednimi kolumnami oraz w drugim wierszu jak na razie potrzebna jest data od której ma pobrać dane (tj. jesli chcemy aby pobrał dane od 2021-01-01 to data musi być 2020-12-31), daily_sum.csv z odpowiednimi kolumnami, gdzie wrzuci różnicę zysku dziennego
...
5. Kolejność uruchamiania plików:
	i) wchodzenie_pobieranie.py - poprosi Cię o podanie loginu i hasła do konta, zaloguje sie do deGiro i pobierze raporty, a na koniec się wyloguje
	ii) wczytywanie_analizowanie - połączy raporty w jedną dużą tabele i dokona obliczeń i zmian dla akcji, wygeneruje raporty dla ostatniego dostępnego dnia i zapisze dane w plikach .csv
