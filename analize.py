...
#Configuration file
cwd = os.getcwd()
config_localization = os.path.join(cwd, 'config.yaml')
config = open(config_localization, 'r')
...
time_last = time_last.date() #date for while loop for files
time_df_max = pd.to_datetime(time_last) #date for dataframes
...
#Lists for function cur_rates, for additional df to take last currency rate if gets error
cur_list_helper = []
rates_helper = []
date_list_helper = []
df_currency_helper = pd.DataFrame()

#Combine all missing files in one dataframe with last data from main file
...
df_deposits = df_deposits.sort_values(by=['Date'])
df_deposits['Date'] = df_deposits['Date'].astype(str)
df_deposits['CumSum'] = df_deposits["Amount"].cumsum()

if df_deposits['Date'].max() != df_group_merge['Data'].min():
    df_depo = df_deposits[df_deposits['Date'] == df_deposits['Date'].max()]
    df_depo.iloc[0, 0] = df_group_merge['Data'].min()
    df_deposits = pd.concat([df_deposits, df_depo])
